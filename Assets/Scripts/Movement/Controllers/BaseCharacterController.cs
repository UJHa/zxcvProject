...
namespace ECM.Controllers
{
    ...
    public class BaseCharacterController : MonoBehaviour
    {
        ...
        protected virtual Vector3 CalcDesiredVelocity()
        {
            ...
            if (character.GetCurState() == eRoleState.WALK
                || character.GetCurState() == eRoleState.RUN)
                moveDirection = GetComponent<Character>().GetDirectionVector();
            else if (character.GetCurState() == eRoleState.IDLE)
            {
        ...
        protected virtual void Move()
        {
            ...
            else
            {
                ...
                Debug.Log($"[testMovement]desiredVelocity({desiredVelocity.magnitude})speed({speed})");
